...
func (a *App) AreaList() (string, tview.Primitive, bool, bool) {
	...
	table.SetSelectedStyle(tcell.ColorWhite, tcell.ColorBlue, tcell.AttrBold)
	table.SetCell(
	...
	table.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		if event.Key() == tcell.KeyEsc {
			...
			log.Print(table.GetSelection())
			a.onSelected(table.GetSelection())
		}
		return event
	})
	for i, ar := range msgapi.Areas {
		...
		table.SetCell(i+1, 1, tview.NewTableCell(ar.GetName()))
		table.SetCell(i+1, 2, tview.NewTableCell(strconv.FormatInt(int64(ar.GetCount()), 10)).SetAlign(tview.AlignRight))
		table.SetCell(i+1, 3, tview.NewTableCell(strconv.FormatInt(int64(ar.GetCount()-ar.GetLast()), 10)).SetAlign(tview.AlignRight))
	}
